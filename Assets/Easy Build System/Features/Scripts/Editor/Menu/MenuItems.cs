            ...
            for (int i = 0; i < Selection.gameObjects.Length; i++)
            {
                if (Selection.gameObjects[i].GetComponentInParent<PieceBehaviour>() == null)
                {
                    ...
                    if (LocalPath != string.Empty)
                    {
                        ...
                        Parent.transform.position = Selection.gameObjects[i].transform.position;
                        Parent.transform.rotation = Selection.gameObjects[i].transform.rotation;

                        Selection.gameObjects[i].transform.SetParent(Parent.transform, true);
