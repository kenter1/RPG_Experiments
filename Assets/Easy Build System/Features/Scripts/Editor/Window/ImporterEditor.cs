...
namespace EasyBuildSystem.Features.Scripts.Editor.Window
{
    ...
    {
        public const string VERSION = "Release 5.3";

        private AddRequest Request;

        private static bool ImportedDemosAddonsRPPackage;
        private static bool ImportedDemosAddonsPackage;
        private static bool ImportedInputAndXRSupport;
        ...
        private void OnGUI()
        {
            ...
            GUILayout.Label("Import here the Demos & Add-Ons and the Input System & XR Toolkit support with all they dependencies.\n" +
            ...
            if (GUILayout.Button("Import Demos & Add-Ons..."))
            {
                if (EditorUtility.DisplayDialog("Easy Build System - Import Package", 
                    "If you have already imported this package and have you made changes and you re-import it, these changes will be liable to be erased.", "Continue", "Cancel"))
                {
                    EditorApplication.update += ProcessDownload;
                    CurrentDownload = Install(false, true);
                }
            }

            if (GUILayout.Button("Import Input System & XR Toolkit Support..."))
            {
                if (EditorUtility.DisplayDialog("Easy Build System - Import Package",
                ...
                {
                    if (EditorUtility.DisplayDialog("Easy Build System - Import Package",
                    "This will import the Input System, XR Interaction Toolkit and XR Plugin Management from the Packager Manager.", "Import", "Cancel"))
                    {
                        EditorApplication.update += ProcessDownload;
                        CurrentDownload = Install(true, false);
                    }
                }
            }
        }
        ...
        private IEnumerator Install(bool importXRandCrossplatform, bool importDemosAddons)
        {
            Logs = null;

            ImportedInputAndXRSupport = false;
            ImportedDemosAddonsRPPackage = false;
            ...
            AddMissingLayers(new string[1] { "Socket" });
            ...
            if (importDemosAddons)
            {
                ...
                if (GraphicsSettings.currentRenderPipeline)
                {
                    if (GraphicsSettings.currentRenderPipeline.GetType().ToString().Contains("HighDefinition"))
                    {
                        ...
                        AssetDatabase.importPackageCompleted += (string packageName) => { ImportedDemosAddonsRPPackage = true; };
                        while (!ImportedDemosAddonsRPPackage)
                        {
                            yield return new WaitForEndOfFrame();
                        }
                    }
            ...
            if (importXRandCrossplatform && Application.internetReachability != NetworkReachability.NotReachable)
            {
                ...
                Logs += "Importing Easy Build System - Input System & XR Support...\n";
                AssetDatabase.ImportPackage("Assets/Easy Build System/Features/Resources/Editor/Packages/Input System & XR Support.unitypackage", false);
                AssetDatabase.importPackageCompleted += (string packageName) => { ImportedInputAndXRSupport = true; };
                while (!ImportedInputAndXRSupport)
                {
                    yield return new WaitForEndOfFrame();
                }
            }
            ...
            Debug.Log("<b>Easy Build System</b> : Installation successfully!");
